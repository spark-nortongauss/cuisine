...
export default async function ShoppingIndexPage() {
  ...
  if (error) {
    console.error("[shopping-page] query error", error);
  }

  const safeLists = lists ?? [];
  console.info("[shopping-page] query result", { count: safeLists.length });
              ...
              <Link key={list.id} href={`/shopping/${list.id}`} className="block">
                <Card className="space-y-2 transition hover:-translate-y-0.5 hover:border-primary/30">
                  <div className="flex items-center justify-between gap-2">
                    <h2 className="font-serif text-2xl">{list.menu_title ?? "Untitled menu"}</h2>
                    <Badge variant={list.status === "purchased" ? "success" : "accent"}>{list.status ?? "pending"}</Badge>
                  </div>
